...
export const SaveConfirmationModal = ({ isOpen, onConfirm, onCancel, isSaving }: SaveConfirmationModalProps) => {
  ...
  return (
    <div className="modal-overlay" onClick={isSaving ? undefined : onCancel}>
      <div className="modal-content save-confirmation-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body"><p>Do you want to save changes to the virtual database?</p></div>
        <div className="modal-footer">
          ...
          <Button variant="primary" onClick={onConfirm} disabled={isSaving}>
            {isSaving ? <><span className="save-modal-spinner" />Saving...</> : 'Yes, Save'}
          </Button>
        </div>
      </div>
    </div>
