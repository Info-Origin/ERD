...
export const SchemaNode = ({ schemaName, tables }: SchemaNodeProps) => {
  ...
  const handleToggle = () => {
    if (!isExpanded) selectSchema(schemaName);
  ...
  return (
    <div className="schema-node">
      ...
      >
        ...
        <span className="schema-name" title={schemaName}>
          {schemaName}
        </span>
      ...
      {isExpanded && tables && (
        <div className="schema-content">
          {Object.entries(tables).map(([tableName, tableData]) => (
            <TableNode key={tableName} tableName={tableName} tableData={tableData} />
          ))}
        </div>
      )}
    </div>
