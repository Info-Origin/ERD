...
export const CanvasControls = ({ isCollapsed }: CanvasControlsProps) => {
  const {
    selectedSchema,
    showFKComparison,
    showNotification,
  ...
  const handleSyncClick = async () => {
    ...
    try {
      ...
      if (hasChanges === false) showNotification?.("No changes to sync. Database is up to date.", "info");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const handleSaveClick = () => {
    if (!selectedSchema) { showNotification?.("Please select a schema first", "warning"); return; }
    if (!hasUnsavedChanges) { showNotification?.("No changes to save", "info"); return; }
  ...
  const handleSaveConfirm = async () => {
    setIsSaving(true);
    try {
      await saveChangesWithDatabaseCheck?.();
    } finally {
      setIsSaving(false);
      ...
      <div className={`canvas-controls ${isCollapsed ? 'collapsed' : ''}`}>
        ...
        >
          {isSyncing ? (
            ...
            <img src="/sync.png" alt="Sync" width="16" height="16"
              style={{ filter: (!selectedSchema || isLockedByOther) ? 'grayscale(100%) opacity(0.5)' : 'none' }} />
          )}
